...
# - Witstats has processed a block in the last 60 seconds
#   (as long as it was also after the container started, we don't want
#    to report healthy immediately after a restart)
# or
# - Witstats's head block has caught up to haf's irreversible block
#   (so we don't mark Witstats as unhealthy if HAF stops getting blocks)
#
...
CHECK="SET TIME ZONE 'UTC'; \
       SELECT ((now() - (SELECT last_active_at FROM hafd.contexts WHERE name = 'witstats_app')) < interval '1 minute' \
               AND (SELECT last_active_at FROM hafd.contexts WHERE name = 'witstats_app') > '${STARTUP_TIME}'::timestamp) OR \
              hive.is_app_in_sync('witstats_app');"
